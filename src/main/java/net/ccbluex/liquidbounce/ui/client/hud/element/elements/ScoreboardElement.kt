...
class ScoreboardElement(
    ...
    // bo tron
    private val roundedRadiusValue = FloatValue("Rounded-Radius", 0F, 0F, 10F)    
    ...
    private val backgroundColorAlphaValue = IntegerValue("Background-Alpha", 150, 0, 255)
    ...
    private val fontValue = FontValue("Font", Fonts.fontTahoma30)

    // ĐÃ CHỈNH SỬA: Bỏ chế độ "Shader" vì logic đã bị xóa. Giờ chỉ còn "None" và "Basic".
    private val shadowModeValue = ListValue("Shadow-Mode", arrayOf("None", "Basic"), "None")

    // ĐÃ XÓA: Toàn bộ thuộc tính cho đổ bóng nâng cao (Shader) đã bị xóa
    /*
    private val shadowStrength = FloatValue("Shadow-Strength", 5.0F, 0F, 30F)
    ...
    private val shadowColorBlueValue = IntegerValue("Shadow-B", 255, 0, 255)
    */
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        val minY = -5f
        val maxX = 9f
        val maxY = maxHeight + fontRenderer.FONT_HEIGHT + 5f
        
        // --- LOGIC ĐỔ BÓNG: BASIC (CƠ BẢN) ---
        if (shadowModeValue.get().equals("Basic", ignoreCase = true)) {
             // Đổ bóng cơ bản (Đổ bóng nhanh, không làm mờ)
             shadowRenderUtils.drawShadowWithCustomAlpha(minX, minY, maxX - minX, maxY - minY, 255f)
        }
        // --- KẾT THÚC LOGIC ĐỔ BÓNG: BASIC (CƠ BẢN) ---

        if(rainbowBarValue.get()) {
            // Thanh rainbow vẫn là hình chữ nhật
            Gui.drawRect(l1 - 7, -6, 9, - 5, ColorUtils.rainbow().rgb)
        }
        
        // Vẽ nền chính với bo tròn
        RenderUtils.drawRoundedRect(minX, minY, maxX, maxY, roundedRadiusValue.get(), backColor)    
        
        // ĐÃ XÓA: Toàn bộ khối logic đổ bóng nâng cao (Shader) đã bị loại bỏ khỏi hàm drawElement.
        ...
        return Border(-maxWidth.toFloat() - 10f - if (rectValue.get()) 3 else 0, -5F, 9F, maxY)
    }
